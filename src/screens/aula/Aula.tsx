...
export default function Aula() {
  ...
  const [DialogListQuestions, openDialogListQuestions] = useDialog(
    ListQuestoes,
    (result: number) => {
      if (result !== null) {
        setPosition(result);
  ...
  const [DialogEdit, openEdit] = useDialog(
    EditQuestao,
    async (result: Questao) => {
      if (result === null) {
        return;
      }
      await loadQuestoes();
  ...
  React.useEffect(() => {
    setQuestoes((old) =>
      old.map((questao, index) => {
        questao.respondidas = respondidas.filter(
          (respondida) => respondida.questao === index
        );
    ...
    const _dias = respondidas
      .reduce<Dia[]>((acc, item) => {
        ...
        return [
          ...acc,
          { data: _curDia, total: 1, acertos: item.acertou ? 1 : 0 },
        ];
      }, [])
      .sort((a, b) => {
        if (a.data.toMillis() > b.data.toMillis()) return -1;
      ...
      old.map((questao, pos) => {
        if (!current?.respondida && pos === position) {
          questao.opcoes = questao.opcoes.map((opcao, index) => {
            if (index === opcao_id) opcao.riscada = !opcao.riscada;
  ...
  const next = () => {
    if (questoes.length >= position - 1) setPosition((old) => old + 1);
  };

  const prev = () => {
    if (position > 0) setPosition((old) => old - 1);
  };
      ...
      <div className="toolbar">
        <div className="toolbar--label">
          <h5 title={aula.name}>
            ...
            {aula.name?.substring(0, 80)}
            {(aula.name?.length || 0) > 80 ? "..." : ""}
          </h5>
          <Link
            className="text-dark"
        ...
        <Col>
          <Card>
            <div className="toolbar toolbar--inner mb-0">
              <div className="stats">
                {current?.respondidas?.map(
                  (item, index) =>
                    (current?.respondidas?.length || 0) - index <= 10 && (
                      <div
                        key={item.id}
                        className={`stat ${item.acertou ? "acertou" : "errou"}`}
                      ></div>
                    )
                )}
                {(current?.respondidas?.length || 0) <= 10 &&
                  Array(10 - (current?.respondidas?.length || 0))
                    .fill("")
                    .map((item, index) => (
              ...
              <div className="actions">
                <a
                  href={`http://deltex.work:9000/index.php?file=${aula.markdown}`}
                  target="_blank"
                  className="btn actions__item"
                >
                  <i className="zmdi zmdi-format-indent-decrease"></i>
                </a>
                <button
                  onClick={() => openEdit({ ...current, aula_id: id })}
                  className="btn actions__item"
                >
                ...
                <button
                  ...
                  className="btn"
                >
                  {position + 1} / {questoes.length}
                </button>
                <button
                  disabled={questoes.length === position + 1}
                  onClick={next}
                  className="btn actions__item"
                >
            ...
            <Card.Body className="p-0">
              ...
              <div className="px-4 pt-3">
                {current?.opcoes?.map((opcao, opcao_id) => (
                  <div
                    key={opcao.letra}
                    style={{ borderBottom: "1px solid #00000005" }}
                    className="d-flex  flex-row align-items-center pb-2 mb-2"
                  >
                    <div className="mr-2">
                      {!current?.respondida && (
                        <Button
                          onClick={() => marcar(opcao_id)}
                          variant={opcao.marcada ? "info" : "light"}
                          style={{ width: 40 }}
                        >
                          {opcao.letra}
                        </Button>
                        ...
                        <Button
                          variant={
                            current?.resposta?.resposta === opcao_id
                              ? current?.resposta?.gabarito === opcao_id
                                ? "success"
                                : "danger"
                              : current?.resposta?.gabarito === opcao_id
                              ? "outline-success"
            ...
            <Card.Body className="d-flex border-top mt-3 px-4 py-3">
              <Button
                onClick={responder}
                disabled={
                  current?.opcoes?.every((it) => !it.marcada) ||
                  current?.respondida
                }
              >
                {loadingResponder ? (
            ...
            {aba === "tempo" && (
              <Card.Body className="p-0">
                <div className="listview listview--bordered">
                  {aula.registros
                    ?.reduce<Registro[]>((acc, item) => {
                      const itemHorario = DateTime.fromISO(item.horario || "");

                      if (
                        acc.some((_acc) =>
                          itemHorario.hasSame(
                            DateTime.fromISO(_acc.horario || ""),
                            "day"
                          )
                        )
                      ) {
                    ...
                    .map((dia) => {
                      dia.tempo = (aula.registros || [])
                        .filter((_dia) =>
                          DateTime.fromISO(_dia.horario || "").hasSame(
                            DateTime.fromISO(dia.horario || ""),
                            "days"
                          )
                        )
                        .reduce((acc, item) => {
                    ...
                    .sort((a, b) => {
                      if (
                        DateTime.fromISO(a.horario || "").toMillis() >
                        DateTime.fromISO(b.horario || "").toMillis()
                      ) {
                        return -1;
                      } else {
                        return 1;
                ...
                <div className="listview listview--bordered">
                  {dias.map((dia) => (
                    <div
                      ...
                      className="listview__item py-3"
                    >
                      <div className="listview__content">
                        <div className="listview-heading">
                          {dia.data.toFormat("dd/MM/yyyy")}
                        </div>
                        <p>
                          {dia.acertos}/{dia.total}
                        </p>
                      </div>
                      <div>
                        <span className="badge badge-light">
                          {Math.floor((dia.acertos / dia.total) * 1000) / 10}%
                        </span>
                      </div>
                    </div>
                  ))}
