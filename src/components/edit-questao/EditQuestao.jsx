...
export default ({ data, onClose }) => {
    ...
    const loadText = async () => {
      try {
        const { data: _data } = await Axios.get(
          `questoes/${data.aula_id}/${data.questao_id || 0}`
        );
  ...
  const onSalvar = async () => {
    setLoading(true);
    try {
      await Axios.post(`questoes/${data.aula_id}/${data.questao_id || 0}`, {
        texto,
      });
  ...
  function handleAction(action) {
    ...
    setTexto(
      (old) => `${inicio}${action.start}${selecionado}${action.end}${final}`
    );
  }
        ...
        setTexto(
          (_texto) =>
            `${_texto.substring(
              0,
              start
            )}![](http://157.245.218.108/uploads/images/${
              data.fileName
            })${_texto.substring(start, _texto.length)}`
        );
      }
    }
  };
